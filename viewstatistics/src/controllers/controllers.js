...
const getQueueLength = async () => {
    try {
        const response = await axios.get(`http://${RABBITMQ_USER}:${RABBITMQ_PASS}@${RABBITMQ_HOST}:15672/api/queues/%2F/${QUEUE_NAME}`);
...
const findAvgExecTime = async () => {
    try {
        ...
        const averageExecTime = result.length > 0 ? parseFloat(result[0].averageExecTime.toFixed(3)) : 0;
...
export const getStats = async (req, res) => {
    try {
        const [totalSubmissions, todaysSubmissions, avgExecTime, data1, data2] = await Promise.all([
            countAll(),
            countTodays(),
            findAvgExecTime(),
            query1(),
        ...
        const data = { totalSubmissions, todaysSubmissions, avgExecTime, data1, data2 };
