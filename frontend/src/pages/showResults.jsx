...
const ShowResults = () => {
  ...
  useEffect(() => {
    ...
    if (isModalOpen && selectedRoute !== null && chartContainerRef.current) {
      ...
      for (let i = 0; i < nodes.length - 1; i++) {
        edges.push({ from: nodes[i], to: nodes[i + 1], color: i === 0 ? 'red' : undefined });
      }
      edges.push({ from: nodes[nodes.length - 1], to: nodes[0], color: undefined }); // Connect last node to the first node
  ...
  return (
    ...
    <br></br><br></br>
    <h2 class="text-2xl font-bold text-blue-500">Information per vehicle : </h2>
    <br></br>
      <div className="money">
        <table>
          <thead>
          ...
          <tbody>
            {routesData.map((route, index) => (
              <tr key={index}>
                ...
                <td>
                  <button className="bg-rose-500 text-white rounded-md px-4 py-2 hover:bg-rose-700 transition" onClick={() => openModal(index)}>
                    Click to see route
                  </button>
                </td>
              </tr>
      ...
      {isModalOpen && selectedRoute !== null && (
        <div className="fixed z-50 inset-0 bg-gray-900 bg-opacity-60 overflow-y-auto h-full w-full px-4">
          <div className="relative top-40 mx-auto shadow-xl rounded-md bg-white max-w-xl">
            ...
            <div ref={chartContainerRef} id="modal-graph" className="p-6 pt-0 text-center"></div>
            <div class="text-center">The path until the first stop is specified with <span class="text-red-500">red</span> color</div>
          </div>
        </div>
      )}
...
{isFileOpen && (
  <div className="fixed z-50 inset-0 bg-gray-900 bg-opacity-60 overflow-y-auto h-full w-full px-4">
    <div className="relative top-40 mx-auto shadow-xl rounded-md bg-white max-w-3xl"> {/* Adjusted max-width */}
      <div className="flex justify-end p-2">
      <button
      ...
      data-ripple-light="true" onClick = {DownloadFile}>
        Download
      </button>
        <button onClick={FileClose} type="button" className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center">
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd"></path>
          </svg>
        </button>
      ...
      <div className="py-3 px-4 flex items-center justify-center bg-gray-800 shadow-2xl rounded-lg overflow-hidden">
        <div className="w-full lg:w-9/12 bg-gray-800 shadow-2xl rounded-lg overflow-hidden"> {/* Adjusted width */}
